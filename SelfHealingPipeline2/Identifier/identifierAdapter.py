...
try:
    ...
    logger.warning("Datadog package not available. Install with: pip install datadog")
...
class GitHubRepositoryIdentifier:
    ...
    def __init__(self, github_token: Optional[str] = None, datadog_api_key: Optional[str] = None, datadog_app_key: Optional[str] = None):
        ...
        if DATADOG_AVAILABLE and self.datadog_api_key and self.datadog_app_key and self.datadog_api_key != "your_datadog_api_key_here":
            try:
            ...
            if not DATADOG_AVAILABLE:
                logger.warning("⚠️  Datadog package not available. Install with: pip install datadog")
            elif not self.datadog_api_key or not self.datadog_app_key or self.datadog_api_key == "your_datadog_api_key_here":
                logger.warning("⚠️  Datadog API keys not provided. Set DATADOG_API_KEY and DATADOG_APP_KEY environment variables.")
    ...
    def identify_issues_and_flags(self, repo_url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Analyze Datadog Alerts (if enabled)
            if self.datadog_enabled:
                datadog_flags = self._analyze_datadog_alerts(owner, repo)
                flags.extend(datadog_flags)
                
                datadog_issues = self._analyze_datadog_metrics(owner, repo)
                issues.extend(datadog_issues)
    ...
    def _analyze_datadog_alerts(self, owner: str, repo: str) -> List[Flag]:
        """Analyze Datadog alerts related to the repository."""
        flags = []
        ...
        try:
            logger.info("🐕 Analyzing Datadog alerts...")
            
            # Query Datadog monitors (alerts) that might be related to this repository
            # In a real implementation, you would query Datadog API for monitors
            # For now, we'll simulate some common alert scenarios
            
            # Mock Datadog alerts for demonstration
            mock_alerts = [
                {
                    "name": f"{owner}-{repo}-api-error-rate",
                    "status": "alert",
                    "message": "High error rate detected in API endpoints",
                    "severity": "high",
                    "tags": [f"repo:{owner}/{repo}", "service:api"]
                },
                {
                    "name": f"{owner}-{repo}-database-connections",
                    "status": "warning", 
                    "message": "Database connection pool near capacity",
                    "severity": "medium",
                    "tags": [f"repo:{owner}/{repo}", "service:database"]
                },
                {
                    "name": f"{owner}-{repo}-memory-usage",
                    "status": "alert",
                    "message": "Memory usage above 90%",
                    "severity": "high",
                    "tags": [f"repo:{owner}/{repo}", "service:app"]
                }
            ]
            
            for alert in mock_alerts:
                # Check if alert is currently firing
                if alert["status"] in ["alert", "warning"]:
                    flag = Flag(
                        id=f"datadog_{alert['name']}_{int(time.time())}",
                        type="datadog_alert",
                        severity=alert["severity"],
                        message=f"Datadog Alert: {alert['message']}",
                        source="datadog",
            ...
            logger.error(f"Failed to analyze Datadog alerts: {str(e)}")
        
        return flags
    
    def _analyze_datadog_metrics(self, owner: str, repo: str) -> List[Issue]:
        """Analyze Datadog metrics for performance and infrastructure issues."""
        issues = []
        ...
        try:
            logger.info("📊 Analyzing Datadog metrics...")
            
            # In a real implementation, you would query Datadog metrics API
            # For now, we'll simulate some common metric-based issues
            ...
            mock_metrics = [
                ...
                {
                    "metric": "database.connection_time",
                    "value": 2.5,
                    "threshold": 1.0,
                    "severity": "medium", 
                    "description": "Database connection time is 2.5s, above threshold of 1s"
                },
                {
                    ...
                    "threshold": 85.0,
                    "severity": "high",
                    "description": "Memory usage is 92.5%, above threshold of 85%"
                }
            ]
            ...
            for metric in mock_metrics:
                if metric["value"] > metric["threshold"]:
                    issue = Issue(
                        id=f"datadog_metric_{metric['metric']}_{int(time.time())}",
                        type="performance_issue",
                        severity=metric["severity"],
                        title=f"Datadog Metric Alert: {metric['metric']}",
                        description=metric["description"],
                        source="datadog_metrics",
                        labels=[f"metric:{metric['metric']}", f"repo:{owner}/{repo}"]
                    )
                    issues.append(issue)
                    
        except Exception as e:
            logger.error(f"Failed to analyze Datadog metrics: {str(e)}")
        
        return issues
    
    def _get_datadog_monitors(self, repo_tag: str) -> List[Dict]:
        """Get Datadog monitors tagged with repository information."""
        if not self.datadog_enabled:
            return []
        
        try:
            # In a real implementation, this would query the Datadog API
            # GET /api/v1/monitor with tags filter
            logger.debug(f"Querying Datadog monitors for repo: {repo_tag}")
            
            # Mock response for demonstration
            return []
            
        except Exception as e:
            logger.error(f"Failed to get Datadog monitors: {str(e)}")
            return []
    
    def _get_datadog_metrics(self, metric_name: str, tags: List[str], time_range: int = 3600) -> Dict:
        """Get Datadog metrics for analysis."""
        if not self.datadog_enabled:
            return {}
        
        try:
            # In a real implementation, this would query the Datadog metrics API
            # GET /api/v1/query with metric name and tags
            logger.debug(f"Querying Datadog metric: {metric_name} with tags: {tags}")
            ...
            logger.error(f"Failed to get Datadog metrics: {str(e)}")
            return {}
